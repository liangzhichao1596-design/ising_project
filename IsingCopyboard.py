...
# --- 主程序执行 ---
# 定义符号
sel0, sel1, sel2, sel3, a0, a1 = symbols('sel0 sel1 sel2 sel3 a0 a1')

# 组合译码器逻辑与唯一性约束
my_expr = And(
    # 译码器等价逻辑
    Or(Not(sel0), Not(a0)), Or(Not(sel0), Not(a1)), Or(sel0, a0, a1),
    Or(Not(sel1), a0), Or(Not(sel1), Not(a1)), Or(sel1, Not(a0), a1),
    Or(Not(sel2), Not(a0)), Or(Not(sel2), a1), Or(sel2, a0, Not(a1)),
    Or(Not(sel3), a0), Or(Not(sel3), a1), Or(sel3, Not(a0), Not(a1)),
    # 唯一性约束
    Or(sel0, sel1, sel2, sel3),
    Or(Not(sel0), Not(sel1)), Or(Not(sel0), Not(sel2)), Or(Not(sel0), Not(sel3)),
    Or(Not(sel1), Not(sel2)), Or(Not(sel1), Not(sel3)), Or(Not(sel2), Not(sel3))
)

steps, h_func, min_e, mapping = translate_to_ising_matrix(my_expr)

# --- 保存到文件并打印 ---
log_filename = "ising_function.log"
