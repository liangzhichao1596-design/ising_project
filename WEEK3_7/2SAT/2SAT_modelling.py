...
class IsingSystem:
    ...
    def add_gate(self, gate_type, inputs, desc):
        # 【核心修改点 1】：强制降维拦截
        # 如果传入了 3 输入门，将其自动拆解为两个 2 输入门级联
        # 例如: OR(A, B, C) -> tmp = OR(A, B); result = OR(tmp, C)
        if len(inputs) == 3:
...
def build_system():
    ...
    for i in range(BIT_WIDTH):
        ...
        # Carry Logic (代码不变，底层的 add_gate 会自动将其拆解为二次项)
...
log_lines.append("=" * 80)
log_lines.append(" ISING MODEL GENERATION REPORT (4-bit RCA vs CLA - QUADRATIC ONLY)")
log_lines.append("=" * 80)
...
# 写入文件
filename = "Full_Ising_Report_Quadratic.log"
with open(filename, "w", encoding="utf-8") as f:
    f.write("\n".join(log_lines))

print(f"成功生成模型！")
print(f"总量子比特数: {model.q_count} (因为降维，相比于三次项模型，所需的辅助比特数量增加了)")
print(f"理论最小能量: {model.min_energy}")
print(f"三次项数量: {len(model.cubic_terms_list)} (已成功消除所有高阶项)")
print(f"\n所有详细数据已写入文件: {filename}")
